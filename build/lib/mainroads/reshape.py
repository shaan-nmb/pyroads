...
def stretch(data, start, end, true_start, true_end, obs_length = 10, dropvars = []):
    ...
    #Same observations for true slk variables if not none 
    if None not in [true_start, true_end]:
        new_data[true_start] = asmetres(new_data[true_start])
        new_data['TRUE_SLK'] = new_data[true_start] + new_data.groupby(level=0).cumcount()*obs_length
