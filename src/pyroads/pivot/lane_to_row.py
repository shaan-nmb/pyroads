...
def lane_to_row(
	...
	if prefixes is None:
			lane_df_base = new_data[id_vars + side + lanes + slks].melt(id_vars = id_vars + side + slks, value_vars = lanes, var_name = lane_var, ignore_index = True)	

	if isinstance(prefixes, str):
		cols = [col for col in new_data.columns if prefixes in col]
		lane_df_base = new_data[cols + id_vars + side + slks].melt(id_vars = id_vars + side, var_name = lane_var, value_name = prefixes, ignore_index = True)

	if isinstance(prefixes, list):
		cols = [col for col in new_data.columns if prefixes[0] in col]
		lane_df_base = new_data[cols + id_vars + [side] + slks].melt(id_vars = id_vars + [side] + slks, var_name = lane_var, value_name = prefixes[0], ignore_index = True)
		lane_df_base = lane_df_base.drop([col for col in lane_df_base if col not in id_vars + slks + side + [lane_var + prefixes[0]]], axis = 1)
	...
	if side_split:
		...
		lane_df_base.insert(len(lane_df_base.columns) -1, 'XSP', lane_df_base[side].astype(str) + lane_df_base.LANE_NO.astype(str))
		lane_df_base['XSP'] = np.where(lane_df_base['XSP'].str.contains('P'), 'TP', lane_df_base['XSP'])
		lane_df = lane_df_base.drop(['LANE_NO', side], axis = 1) 
